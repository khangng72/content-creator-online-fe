...
import { generateApi, GET_FOLLOWING_BY_USERID } from '@/constants/api';
...
const FollowingList = ({ userData, isOpen }: FollowerListProps) => {
  const { ref, inView } = useInView();

  const fetchFollowing = async ({ pageParam }: { pageParam: number }) => {
    ...
    try {
      const response = await axios.get(
        generateApi(GET_FOLLOWING_BY_USERID, userData.id, `page=${pageParam}`),
        {
          headers,
        }
      );
